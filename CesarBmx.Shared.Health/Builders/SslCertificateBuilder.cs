...
using CesarBmx.Shared.Health.Expressions;
using CesarBmx.Shared.Health.Models;

namespace CesarBmx.Shared.Health.Builders
{
    public static class SslCertificateBuilder 
    {
        public static HealthCheckResult BuildResult(SslCertificate sslCertificate)
        {
            ...
            if (SslCertificateExpression.CertificateMissing().Invoke(sslCertificate))
            {
                return HealthCheckResult.Unhealthy($"SSL certificate is missing on {sslCertificate.Url}");
            }

            // Certificate already expired 
            if (SslCertificateExpression.CertificateAlreadyExpired().Invoke(sslCertificate))
            {
            ...
            if (SslCertificateExpression.CertificateIsNotValidYet().Invoke(sslCertificate))
            {
            ...
            if (SslCertificateExpression.CertificateAboutToExpire(60).Invoke(sslCertificate))
            {
            ...
            if (SslCertificateExpression.CertificateHasNoIssuer().Invoke(sslCertificate))
            {
                return HealthCheckResult.Degraded("SSL certificate has no issuer");
            }

            // Certificate has no subject
            if (SslCertificateExpression.CertificateHasNoSubject().Invoke(sslCertificate))
            {
                return HealthCheckResult.Degraded("SSL certificate has no subject");
            }

            // Certificate is no greater than v2
            if (SslCertificateExpression.CertificateDoesNotHaveVersionGreaterThan2().Invoke(sslCertificate))
            {
